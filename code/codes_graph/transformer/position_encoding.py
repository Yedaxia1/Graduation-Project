...
class LapEncoding(PositionEncoding):
    ...
    def compute_pe(self, graph):
        ...
        # 防止节点数小于lap_dim
        if self.pos_enc_dim >= EigVec.shape[1]:
            padding_zero = torch.zeros(EigVec.shape[0], self.pos_enc_dim)
            padding_zero[:, :EigVec.shape[1] - 1] = torch.from_numpy(EigVec[:, 1:self.pos_enc_dim+1])
            return padding_zero.float()
